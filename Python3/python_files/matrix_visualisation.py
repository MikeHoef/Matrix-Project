...
def hasse(name=None, Matrices=[], Relations=None, view=True, orient='BT', splines='true', ranksep=1, penwidth=1, arrowhead='none', nodesep=1,highlighted=[]):
    ...
    Source = graphviz_hasse(Matrices, rels, splines=splines,ranksep=ranksep,nodesep=nodesep, arrowhead=arrowhead, penwidth=penwidth, highlighted=highlighted)
    graph = graphviz.Source(Source)

    if name == None:
        graph.view(directory='./python_files/temp', cleanup=True)
    else:
        graph.render(name, './python_files/pictures', view=view, format='png', cleanup=True)
