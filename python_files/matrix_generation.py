...
def mclex(n,m,k):
    ...
    if os.path.isfile(path):
        ...
        size  = len(Matrices)
        T = time.time()
        Relations = set(relations(n,m,k))       # used to minimize calls to imlpies method.
...
def relations(n,m,k):
    ...
    return relations(n-1,m,k) + relations(n,m-1,k) + relations(n,m,k-1)
